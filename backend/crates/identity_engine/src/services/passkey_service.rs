...
impl PasskeyService {
    ...
    pub fn new(
        ...
        let url_origin = Url::parse(origin).map_err(|e| format!("Invalid origin URL: {e}"))?;
    ...
    pub async fn start_registration(
        ...
        let webauthn_namespace =
            Uuid::parse_str("6ba7b810-9dad-11d1-80b4-00c04fd430c8").unwrap_or(Uuid::nil()); // DNS namespace UUID
        let webauthn_user_id = Uuid::new_v5(&webauthn_namespace, user_id.as_bytes());

        // Start registration
        let (ccr, reg_state) = self
            .webauthn
            .start_passkey_registration(webauthn_user_id, email, email, exclude_credentials)
    ...
    pub async fn finish_authentication(
        ...
        let auth_result = self
            ...
            .map_err(|e| {
                AppError::BadRequest(format!("Authentication verification failed: {e:?}"))
            })?;
        ...
        tracing::info!(user_id = user_id, "Passkey authentication successful");
