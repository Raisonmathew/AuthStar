...
async fn test_create_organization(pool: PgPool) {
    ...
    let org = service
        .create_organization("user_1", "Acme Corp", None)
        .await
        .expect("create org");
...
async fn test_create_duplicate_slug(pool: PgPool) {
    ...
    service
        .create_organization("user_1", "Acme Corp", Some("acme"))
        .await
        .expect("create org 1");

    let result = service
        .create_organization("user_2", "Acme Inc", Some("acme"))
        .await;
...
async fn test_add_remove_member(pool: PgPool) {
    ...
    // Add new member
    let member = service
        .add_member(&org.id, "user_2", "member")
        .await
        .expect("add member");
...
async fn test_cannot_remove_last_admin(pool: PgPool) {
    ...
    match result {
        Err(AppError::BadRequest(msg)) => {
            assert_eq!(msg, "Cannot remove the last admin from organization")
        }
    ...
    // Add another admin
    service
        .add_member(&org.id, "user_2", "admin")
        .await
        .expect("add admin");

    // Now removal should succeed
    service
        .remove_member(&org.id, "user_1")
        .await
        .expect("remove admin");
}
