...
async fn run() -> Result<()> {
        ...
        .await
    {
        Ok(root_pool) => {
            ...
            let row: Option<(i32,)> =
                sqlx::query_as("SELECT 1 FROM pg_roles WHERE rolname = 'idaas_user'")
            ...
            if row.is_none() {
                ...
                if let Err(e) =
                    sqlx::query("CREATE USER idaas_user WITH PASSWORD 'password' SUPERUSER LOGIN")
                        .execute(&root_pool)
                        .await
                {
            ...
            let row: Option<(i32,)> =
                sqlx::query_as("SELECT 1 FROM pg_database WHERE datname = 'idaas'")
                    .fetch_optional(&root_pool)
                    .await
                    .unwrap_or(None);
            ...
            if row.is_none() {
                ...
                if let Err(e) = sqlx::query("CREATE DATABASE idaas OWNER idaas_user")
                    .execute(&root_pool)
                    .await
                {
    ...
    let m = sqlx::migrate::Migrator::new(std::path::Path::new("crates/db_migrations/migrations"))
        .await?;
    m.run(&pool).await?;
