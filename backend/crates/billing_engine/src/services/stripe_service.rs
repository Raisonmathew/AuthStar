...
impl StripeService {
    ...
    fn headers(&self) -> Result<HeaderMap> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {}", self.secret_key))
                .map_err(|_| AppError::Internal("Bad key".into()))?,
        );
        headers.insert(
            CONTENT_TYPE,
    ...
    pub async fn create_checkout_session(
        ...
        let customer_id = self
            .get_or_create_customer_id(org_id, customer_email)
            .await?;
        ...
        json["url"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| AppError::Internal("No URL in response".into()))
    }
    ...
    pub fn verify_signature(
        ...
        let t = timestamp.ok_or_else(|| {
            AppError::Unauthorized("Missing timestamp in Stripe-Signature".into())
        })?;
        ...
        let ts = t.parse::<i64>().map_err(|_| {
            AppError::Unauthorized("Invalid timestamp format in Stripe-Signature".into())
        })?;
    ...
    pub async fn list_subscriptions(&self, org_id: &str) -> Result<Vec<serde_json::Value>> {
        ...
        let subs = json["data"].as_array().cloned().unwrap_or_default();
    ...
    pub async fn list_invoices(&self, org_id: &str, limit: u32) -> Result<Vec<serde_json::Value>> {
        ...
        let invoices = json["data"].as_array().cloned().unwrap_or_default();
