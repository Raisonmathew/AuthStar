-- Migration 036: Session EIAA Decision Reference
--
-- RENAMED from 032_session_decision_ref.sql to resolve migration prefix collision.
-- The original file shared the '032_' prefix with 032_backfill_capsule_bytes.sql (now 035),
-- causing non-deterministic execution order in sqlx migrate.
--
-- Execution order dependency:
--   033_reconcile_eiaa_schema.sql → ensures eiaa_executions has decision_ref column
--   035_backfill_capsule_bytes.sql → backfill scaffolding
--   036_session_decision_ref.sql  → adds decision_ref to sessions (depends on 033 schema)
--
...
COMMENT ON COLUMN sessions.decision_ref IS
    ...
    'NULL for sessions created before EIAA enforcement was enabled (pre-036 records) '
