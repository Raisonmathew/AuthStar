...
async fn test_stripe_webhook_status_update(pool: PgPool) {
    ...
    // Verify status
    let status: String =
        sqlx::query_scalar("SELECT status FROM stripe_webhook_events WHERE event_id = $1")
            .bind(event_id)
            .fetch_one(&pool)
            .await
            .expect("Query failed");
...
async fn test_account_lockout_fields(pool: PgPool) {
    ...
    // Verify count
    let attempts: i32 = sqlx::query_scalar("SELECT failed_login_attempts FROM users WHERE id = $1")
        .bind(user_id)
        .fetch_one(&pool)
        .await
        .expect("Query failed");
    ...
    // Verify locked
    let locked: bool = sqlx::query_scalar("SELECT locked FROM users WHERE id = $1")
        .bind(user_id)
        .fetch_one(&pool)
        .await
        .expect("Query failed");
...
async fn test_duplicate_email_same_tenant_rejected(pool: PgPool) {
    ...
    assert!(
        result.is_err(),
        "Duplicate email in same tenant should be rejected"
    );
}
...
async fn test_password_history_table_exists(pool: PgPool) {
    ...
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM password_history WHERE user_id = $1")
