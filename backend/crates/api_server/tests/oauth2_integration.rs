...
async fn seed_oauth_client(
    ...
    let uris_json: Vec<Value> = redirect_uris.iter().map(|u| Value::String(u.to_string())).collect();
    let flows_json: Vec<Value> = allowed_flows.iter().map(|f| Value::String(f.to_string())).collect();
...
async fn setup_oauth_fixtures(pool: &PgPool) {
    ...
    seed::seed_identity(pool, "id_oauth_1", "user_oauth_1", "oauth@test.com", "org_oauth_test").await;
    seed::seed_password(pool, "user_oauth_1", "TestPassword123!").await;
    seed::seed_membership(pool, "mem_oauth_1", "user_oauth_1", "org_oauth_test", "member").await;
...
async fn test_openid_configuration_returns_valid_document(pool: PgPool) {
    ...
    assert!(body["authorization_endpoint"].as_str().unwrap().contains("/oauth/authorize"));
    assert!(body["token_endpoint"].as_str().unwrap().contains("/oauth/token"));
    assert!(body["userinfo_endpoint"].as_str().unwrap().contains("/oauth/userinfo"));
    assert!(body["jwks_uri"].as_str().unwrap().contains("/.well-known/jwks.json"));
    assert!(body["revocation_endpoint"].as_str().unwrap().contains("/oauth/revoke"));
    assert!(body["introspection_endpoint"].as_str().unwrap().contains("/oauth/introspect"));
    ...
    assert!(body["scopes_supported"].as_array().unwrap().iter().any(|v| v == "openid"));
}
...
async fn test_jwks_returns_ec_key(pool: PgPool) {
    ...
    assert_eq!(x.len(), 43, "x coordinate should be 43 base64url chars (32 bytes)");
    assert_eq!(y.len(), 43, "y coordinate should be 43 base64url chars (32 bytes)");
}
...
async fn test_authorize_rejects_invalid_redirect_uri(pool: PgPool) {
    ...
    let body: Value = res.json().await.unwrap();
    assert_eq!(body["error"], "invalid_request");
    assert!(body["error_description"].as_str().unwrap().contains("redirect_uri"));
}
...
async fn test_client_credentials_grant(pool: PgPool) {
    ...
    assert_eq!(res.status(), StatusCode::OK);
    let cache_control = res.headers().get("cache-control").map(|v| v.to_str().unwrap().to_string());
    let body: Value = res.json().await.unwrap();
    assert!(body["access_token"].is_string(), "Missing access_token");
    assert_eq!(body["token_type"], "Bearer");
    assert!(body["expires_in"].as_i64().unwrap() > 0);
    assert!(body["refresh_token"].is_null(), "client_credentials must NOT issue refresh token");
...
async fn test_authorization_code_grant_full_flow(pool: PgPool) {
    ...
    let code = oauth_service.create_authorization_code(code_ctx).await.unwrap();

    // Consume the auth context (as grant_consent would)
    oauth_service.consume_authorization_context(&flow_id).await.unwrap();
    ...
    assert_eq!(body["token_type"], "Bearer");
    assert!(body["expires_in"].as_i64().unwrap() > 0);
    assert!(body["refresh_token"].is_string(), "Should issue refresh token");
...
async fn test_authorization_code_grant_with_pkce(pool: PgPool) {
    ...
    let code = h.state.oauth_as_service.create_authorization_code(code_ctx).await.unwrap();
...
async fn test_refresh_token_rotation(pool: PgPool) {
    ...
    assert_eq!(res4.status(), StatusCode::BAD_REQUEST, "Family should be revoked after reuse");
}
...
async fn test_refresh_token_scope_narrowing(pool: PgPool) {
    ...
    assert_eq!(res2.status(), StatusCode::OK);
    let body2: Value = res2.json().await.unwrap();
    assert_eq!(body2["scope"], "openid", "Scope should be narrowed to openid only");
}
...
async fn test_consent_grant_and_check(pool: PgPool) {
    ...
    // No consent yet
    let has_consent = svc.check_consent("user_oauth_1", "client_test_001", "org_oauth_test", "openid profile").await.unwrap();
    assert!(!has_consent, "Should have no consent yet");

    // Grant consent
    svc.grant_consent("user_oauth_1", "client_test_001", "org_oauth_test", "openid profile email", None).await.unwrap();
    ...
    let has_consent = svc.check_consent("user_oauth_1", "client_test_001", "org_oauth_test", "openid profile offline_access").await.unwrap();
    assert!(!has_consent, "Superset of consented scopes should fail");

    // Re-consent with narrower scope (upsert)
    svc.grant_consent("user_oauth_1", "client_test_001", "org_oauth_test", "openid", None).await.unwrap();
    let has_consent = svc.check_consent("user_oauth_1", "client_test_001", "org_oauth_test", "openid profile").await.unwrap();
    assert!(!has_consent, "Re-consent with narrower scope should not cover old scopes");
}
...
async fn test_consent_revocation_revokes_tokens(pool: PgPool) {
    ...
    svc.grant_consent("user_oauth_1", "client_test_001", "org_oauth_test", "openid", None).await.unwrap();
    let raw_token = svc.create_refresh_token(
        "client_test_001", "user_oauth_1", "sess_oauth_1", "org_oauth_test",
        "openid", 86400, None, None, None,
    ).await.unwrap();
    ...
    let raw_token2 = svc.create_refresh_token(
        "client_test_001", "user_oauth_1", "sess_oauth_1", "org_oauth_test",
        "openid", 86400, None, None, None,
    ).await.unwrap();

    // Revoke consent — should revoke all tokens for this client+user
    svc.revoke_consent("user_oauth_1", "client_test_001", "org_oauth_test").await.unwrap();

    // Token should be invalid
    let rt2 = svc.consume_refresh_token(&raw_token2).await.unwrap();
    assert!(rt2.is_none(), "Token should be revoked after consent revocation");
}
...
fn test_redirect_uri_exact_match() {
    ...
    assert!(OAuthAsService::validate_redirect_uri(&app, "https://example.com/callback"));
    assert!(!OAuthAsService::validate_redirect_uri(&app, "https://example.com/callback/"));
    assert!(!OAuthAsService::validate_redirect_uri(&app, "https://evil.com/callback"));
