...
async fn test_admin_signup_flow(pool: PgPool) -> anyhow::Result<()> {
    ...
    let init_res = client
        .post(format!("{base_url}/api/hosted/auth/flows"))
        .json(&json!({
            "org_id": "system",
    ...
    let submit_creds_res = client
        .post(format!("{base_url}/api/hosted/auth/flows/{flow_id}/submit"))
        .json(&json!({
            "type": "credentials",
            "value": {
                "email": email,
    ...
    if verify_status != StatusCode::OK {
        panic!(
            "Verify step - Status: {}, Body: {:?}",
            verify_status,
            String::from_utf8_lossy(&verify_body_bytes)
        );
    }
