...
pub fn compile_conditions(
    ...
    for (i, cond) in conditions.iter().enumerate() {
        ...
        let params = cond.condition_params.clone().unwrap_or(serde_json::json!({}));
...
fn normalise_condition_params(
    ...
    match condition_type {
        ...
        "aal_below" => {
            let level = params.get("level")
                ...
                .ok_or_else(|| AppError::BadRequest(
                    "Condition 'aal_below': 'level' must be an integer".into()
                ))?;
        ...
        "impossible_travel" => {
            let speed = params.get("max_speed_kmh")
                .and_then(|v| v.as_f64())
                .ok_or_else(|| AppError::BadRequest(
        ...
        "custom_claim" => {
            let key = params.get("claim_key")
                .and_then(|v| v.as_str())
                .ok_or_else(|| AppError::BadRequest(
                    "Condition 'custom_claim': 'claim_key' must be a string".into()
                ))?;

            let value = params.get("claim_value")
                .and_then(|v| v.as_str())
                .ok_or_else(|| AppError::BadRequest(
...
fn evaluate_single_condition(
    ...
    let params = condition.get("params").cloned().unwrap_or(serde_json::json!({}));

    match ct {
        "risk_above" => {
            let threshold = params.get("threshold").and_then(|v| v.as_f64()).unwrap_or(100.0);
        ...
        "country_in" => {
            let countries: Vec<String> = params.get("countries")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_uppercase())).collect())
                .unwrap_or_default();
            ctx.country_code.as_ref()
        ...
        "outside_time_window" => {
            ...
            let end   = params.get("end_hour").and_then(|v| v.as_i64()).unwrap_or(23);
        ...
        "role_in" => {
            let roles: Vec<String> = params.get("roles")
                .and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_lowercase())).collect())
                .unwrap_or_default();
            ctx.user_roles.iter().any(|r| roles.contains(&r.to_lowercase()))
        }
        "role_not_in" => {
            let roles: Vec<String> = params.get("roles")
                .and_then(|v| v.as_array())
        ...
        "custom_claim" => {
            let key   = params.get("claim_key").and_then(|v| v.as_str()).unwrap_or("");
            let value = params.get("claim_value").and_then(|v| v.as_str()).unwrap_or("");
            ctx.custom_claims
                .get(key)
                .map(|v| v == value)
                .unwrap_or(false)
...
mod tests {
    ...
    fn test_evaluate_risk_above() {
        ...
        let ctx = SimulationContext { risk_score: Some(85.0), ..Default::default() };
        assert!(evaluate_conditions(&conditions, &ctx));

        let ctx_low = SimulationContext { risk_score: Some(50.0), ..Default::default() };
    ...
    fn test_evaluate_or_chain() {
        ...
        let ctx_vpn = SimulationContext { vpn_detected: Some(true), ..Default::default() };
        assert!(evaluate_conditions(&conditions, &ctx_vpn));

        let ctx_tor = SimulationContext { tor_detected: Some(true), ..Default::default() };
