...
impl CustomDomainService {
    ...
    pub async fn get_domain(&self, id: &str) -> Result<CustomDomain> {
        let row = sqlx::query_as::<_, (String, String, String, String, String, Option<DateTime<Utc>>, 
                                       String, bool, bool, DateTime<Utc>)>(r#"
    ...
    pub async fn list_domains(&self, org_id: &str) -> Result<Vec<CustomDomain>> {
        let rows = sqlx::query_as::<_, (String, String, String, String, String, Option<DateTime<Utc>>, 
                                        String, bool, bool, DateTime<Utc>)>(r#"
            SELECT id, organization_id, domain, verification_status, verification_token,
                   verified_at, ssl_status, is_primary, is_active, created_at
        ...
        Ok(rows.into_iter().map(|row| CustomDomain {
            id: row.0,
            organization_id: row.1,
            domain: row.2,
            verification_status: row.3,
    ...
    pub async fn verify_domain(&self, id: &str) -> Result<bool> {
        ...
        "#)
            ...
            .bind(id)
            .bind(verified)
            .bind(if verified { None } else { Some("DNS record not found") })
            .execute(&self.db)
            .await?;
        ...
        match resolver.txt_lookup(&verification_domain).await {
            Ok(txt_records) => {
                ...
                tracing::debug!(
                    ...
                    verification_domain, expected_token
                );
                Ok(false)
            }
            Err(e) => {
...
mod tests {
    ...
    fn test_domain_validation() {
        ...
        assert!(CustomDomainService::is_valid_domain("my-auth.example.co.uk"));
