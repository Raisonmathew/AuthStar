...
mod tests {
    ...
    fn test_compute_achieved_aal() {
        ...
        assert_eq!(
            svc.compute_achieved_aal(&[Capability::Password]),
            AssuranceLevel::AAL1
        );
        assert_eq!(
            svc.compute_achieved_aal(&[Capability::Password, Capability::Totp]),
            AssuranceLevel::AAL2
        );
        assert_eq!(
            svc.compute_achieved_aal(&[Capability::PasskeyHardware]),
            AssuranceLevel::AAL3
        );
    }
    ...
    fn test_suggest_next_capabilities() {
        ...
        let suggestions =
            svc.suggest_next_capabilities(AssuranceLevel::AAL2, &verified, &acceptable);
