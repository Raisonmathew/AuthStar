...
impl UserLocationService {
    ...
    pub async fn get_recent_logins(
        ...
        let rows = sqlx::query_as::<_, (String, Option<String>, Option<String>, 
                                        Option<f64>, Option<f64>, DateTime<Utc>)>(r#"
        ...
        Ok(rows.into_iter().map(|(ip, country, city, lat, lon, ts)| {
            (GeoLocation {
                ip_address: ip,
    ...
    pub async fn analyze_velocity(
        ...
        match (current.latitude, current.longitude, last_loc.latitude, last_loc.longitude) {
            (Some(lat1), Some(lon1), Some(lat2), Some(lon2)) => {
                ...
                let is_impossible_travel = velocity_km_h
                    .map(|v| v > 1000.0)
                    .unwrap_or(false);
    ...
    pub async fn get_baseline(&self, user_id: &str) -> anyhow::Result<Option<UserGeoBaseline>> {
        let row = sqlx::query_as::<_, (String, Option<String>, Option<String>, 
                                       serde_json::Value, Option<f64>, Option<f64>,
                                       Option<f64>, i32)>(r#"
        ...
        Ok(row.map(|(uid, pc, pcity, cc, lat, lon, dist, count)| {
            UserGeoBaseline {
                user_id: uid,
                primary_country: pc,
                primary_city: pcity,
