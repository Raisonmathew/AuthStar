...
impl BaselineComputationJob {
    ...
    pub async fn compute_baseline(&self, user_id: &str) -> anyhow::Result<()> {
        ...
        let common_countries: Vec<String> = sqlx::query_scalar(
            r#"
            SELECT country_code
            FROM user_geo_history
            WHERE user_id = $1 AND success = true AND country_code IS NOT NULL
        ...
        let common_ips: Vec<String> = sqlx::query_scalar(
            r#"
            SELECT ip_address
            FROM user_geo_history
            WHERE user_id = $1 AND success = true
        ...
        let locations: Vec<(Option<f64>, Option<f64>)> = sqlx::query_as(
            r#"
        ...
        let max_distance = locations
            .iter()
            .enumerate()
            .flat_map(|(i, a)| locations[i + 1..].iter().map(move |b| (a, b)))
            .filter_map(
                |((lat1, lon1), (lat2, lon2))| match (lat1, lon1, lat2, lon2) {
                    (Some(la1), Some(lo1), Some(la2), Some(lo2)) => Some(
                        crate::signals::location::haversine_distance(*la1, *lo1, *la2, *lo2),
                    ),
                    _ => None,
                },
            )
            .fold(None, |max: Option<f64>, d| {
                Some(max.map_or(d, |m: f64| m.max(d)))
            });
