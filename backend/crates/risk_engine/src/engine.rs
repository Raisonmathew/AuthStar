...
use shared_types::{AssuranceLevel, RiskConstraints, RiskContext};

use crate::decay::{RiskDecayService, StabilizingEvent};
use crate::rules::RulesEngine;
use crate::scoring::RiskScorer;
use crate::signals::{NetworkInput, SignalCollector, WebDeviceInput};
...
impl RiskEngine {
    ...
    pub async fn evaluate(
        ...
        let signals = self
            .signal_collector
            .collect(&request.network, request.device.as_ref(), user_id)
            .await;
    ...
    async fn persist_risk_signals(&self, user_id: &str, signals: &crate::signals::RawSignals) {
        use crate::decay::{DecayModel, RiskStateEntry};
        ...
        if matches!(
            ...
            shared_types::GeoVelocity::Impossible
        ) {
            let entry = RiskStateEntry {
        ...
        if matches!(signals.device.trust, shared_types::DeviceTrust::New) {
            let entry = RiskStateEntry {
                ...
                first_seen: now,
                last_seen: now,
                decay_model: DecayModel::Temporal {
    ...
    pub async fn quick_evaluate(&self, subject_id: &str) -> RiskEvaluation {
    ...
    pub async fn on_successful_auth(&self, subject_id: &str, achieved_aal: AssuranceLevel) {
        ...
        self.decay_service
            .apply_stabilizing_event(subject_id, event, achieved_aal)
            .await;
    }
    ...
    async fn store_evaluation(&self, subject_id: &str, eval: &RiskEvaluation) {
        let risk_value: serde_json::Value = serde_json::to_value(&eval.risk).unwrap_or_else(|e| {
