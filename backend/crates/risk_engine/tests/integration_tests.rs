...
use shared_types::{
    ...
    DeviceTrust, IpReputation, GeoVelocity, AccountStability, RiskLevel,
};
...
mod risk_scoring {
    ...
    fn test_high_risk_flow() {
        ...
        assert!(constraints.disallowed_capabilities.contains(&Capability::SmsOtp));
        assert!(constraints.require_phishing_resistant);
    }
...
mod capability_filtering {
    ...
    fn test_compromised_device_disables_password() {
        ...
        assert!(constraints.disallowed_capabilities.contains(&Capability::Password));
        assert!(constraints.disallowed_capabilities.contains(&Capability::SmsOtp));
        assert!(constraints.disallowed_capabilities.contains(&Capability::EmailOtp));
    }
...
mod device_trust_rules {
    ...
    fn test_device_trust_to_aal() {
        ...
        assert_eq!(DeviceTrust::Unknown.min_required_aal(), AssuranceLevel::AAL2);
        assert_eq!(DeviceTrust::Changed.min_required_aal(), AssuranceLevel::AAL2);
        assert_eq!(DeviceTrust::Compromised.min_required_aal(), AssuranceLevel::AAL3);
    }
...
mod capability_amr_mapping {
    ...
    fn test_amr_to_capability() {
        ...
        assert_eq!(Capability::from_amr("fido"), Some(Capability::PasskeyHardware));
        assert_eq!(Capability::from_amr("fido2"), Some(Capability::PasskeyHardware));
