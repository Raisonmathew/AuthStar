...
impl Keystore for MockKeystore {
    ...
    fn generate_ed25519(&self) -> Result<KeyId> { Ok(KeyId("mock-kid".to_string())) }
    fn import_ed25519(&self, _sk_bytes: &[u8]) -> Result<KeyId> { Ok(KeyId("mock-kid".to_string())) }
    fn public_key(&self, kid: &KeyId) -> Result<keystore::PublicKey> { 
        Ok(keystore::PublicKey { kid: kid.clone(), key: self.verifier() })
    }
    fn list_public_keys(&self) -> Vec<keystore::PublicKey> { 
        vec![keystore::PublicKey { kid: KeyId("mock-kid".to_string()), key: self.verifier() }] 
    }
}
...
fn test_vector_1_simple_allow() -> Result<()> {
    let program = ast::Program {
        ...
        sequence: vec![
            ...
            Step::AuthorizeAction { action: "read".to_string(), resource: "profile".to_string() },
            Step::Allow(true),
        ],
    };
    ...
    let expected_wasm_hash = "6e944a009cbaf58b78406c1bf62cfffe6fa34a5b6c8241d4e1dea95389556442"; // Likely stale
...
fn test_vector_2_risk_step_up() -> Result<()> {
    let program = ast::Program {
        ...
        sequence: vec![
            ...
            Step::Conditional { 
                condition: Condition::RiskScore { comparator: Comparator::Gt, value: Some(80) },
                then_branch: vec![ Step::RequireFactor { factor_type: FactorType::Otp } ],
                else_branch: None, // Implicit continue
            },
            Step::AuthorizeAction { action: "transfer".to_string(), resource: "bank".to_string() },
            Step::Allow(true),
        ],
    };

    let expected_ast_hash = "e9c94829aab9b50bd18188c8bd97ffac82fec990902a7ad8d8c4cc638948ec7a"; 
    let expected_wasm_hash = "620dfb86c0db6cd3400e0cd93de8aad5a20ef27560d75a37f1f20a8825a34cfe";
    ...
    assert_eq!(capsule.wasm_hash, expected_wasm_hash, "V2 WASM Hash Mismatch");
...
fn test_vector_3_risk_deny() -> Result<()> {
    let program = ast::Program {
        ...
        sequence: vec![
            Step::VerifyIdentity { source: IdentitySource::Primary },
            Step::EvaluateRisk { profile: "strict".to_string() },
            Step::Conditional { 
                condition: Condition::RiskScore { comparator: Comparator::Gt, value: Some(90) },
    ...
    assert_eq!(capsule.ast_hash, "dfe48e63be874e546880cc4e649b12f7ca283ea419e310a6388bfed99243d2dc", "V3 AST Hash Mismatch");
    assert_eq!(capsule.wasm_hash, "7f5323b5b86dac78749003e039c6b36f8ad7e85cded9d3bf40da1d294b16d16f", "V3 WASM Hash Mismatch");
    ...
    let expected_wasm_hash = "7f5323b5b86dac78749003e039c6b36f8ad7e85cded9d3bf40da1d294b16d16f";
...
fn test_vector_4_authz_denial() -> Result<()> {
    let program = ast::Program {
        ...
        sequence: vec![
            Step::VerifyIdentity { source: IdentitySource::Primary },
            Step::AuthorizeAction { action: "sys".to_string(), resource: "root".to_string() },
            Step::Conditional {
                condition: Condition::AuthzResult { comparator: Comparator::Eq, value: Some(0) },
                then_branch: vec![ Step::Deny(true) ],
                else_branch: None
            },
            Step::Allow(true),
    ...
    let expected_wasm_hash = "6d82c6263dc3c70a212e34203952d013cf092eec47c1fbe6bca48a63c7eb79d8";
    ...
    assert_eq!(capsule.wasm_hash, expected_wasm_hash, "V4 WASM Hash Mismatch");
...
fn test_vector_6_nesting() -> Result<()> {
    let program = ast::Program {
        ...
        sequence: vec![
            Step::VerifyIdentity { source: IdentitySource::Primary },
            Step::EvaluateRisk { profile: "def".to_string() },
            Step::Conditional {
                ...
                then_branch: vec![
                    Step::Conditional {
                        condition: Condition::RiskScore { comparator: Comparator::Gt, value: Some(90) },
                        then_branch: vec![ Step::Deny(true) ],
                        else_branch: Some(vec![ Step::RequireFactor { factor_type: FactorType::Otp } ])
                    }
                ],
                else_branch: None,
            },
    ...
    assert_eq!(capsule.ast_hash, "1c2bb124052293243ef3392878d571c372a64b0d11f0dd23e4cb0cd11d487322", "V6 AST Hash Mismatch");
    assert_eq!(capsule.wasm_hash, "a7b835de1ed3b78cc4bf3ed14d208093fc11293020505c4f89e01f2fc2cc5182", "V6 WASM Hash Mismatch");
    ...
    let expected_wasm_hash = "a7b835de1ed3b78cc4bf3ed14d208093fc11293020505c4f89e01f2fc2cc5182";
