...
export function TemplatePicker({ templates, onSelect, onClose }: TemplatePickerProps) {
  ...
  return (
    <>
      ...
      {/* Slide-over panel */}
      <div
        className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-slate-900 border-l border-slate-800 flex flex-col shadow-2xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
          <div>
            <h2 className="text-base font-semibold text-white">Choose a Rule Template</h2>
        ...
        <div className="px-4 py-3 border-b border-slate-800">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {sortedCategories.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <p className="text-slate-400 text-sm">No templates match "{search}"</p>
            </div>
          ) : (
