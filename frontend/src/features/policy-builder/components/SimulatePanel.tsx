...
export function SimulatePanel({ configId }: SimulatePanelProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-base font-semibold text-foreground">Simulate Policy</h3>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {PRESETS.map((p) => (
          <button
            ...
            className="px-3 py-1 text-xs bg-accent border border-border rounded-full text-foreground hover:border-primary hover:text-primary transition-colors"
          >
            {p.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 bg-card border border-border rounded-2xl p-5">
          <h4 className="text-sm font-semibold text-foreground">Test Context</h4>

          {/* Risk score */}
          <div className="space-y-1">
            <div className="flex justify-between">
              <label className="text-xs font-medium text-foreground">Risk Score</label>
              <span className="text-xs text-muted-foreground font-mono">{context.risk_score ?? 0}</span>
            </div>
            <input
              type="range"
              min={0}
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="US"
              maxLength={2}
              className="w-full bg-muted border border-border rounded-lg px-3 py-1.5 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring font-mono uppercase"
            />
          </div>

          {/* AAL level */}
          <div className="space-y-1">
            <label className="text-xs font-medium text-foreground">AAL Level</label>
          ...
          <div className="space-y-1">
            <div className="flex justify-between">
              <label className="text-xs font-medium text-foreground">Current Hour (0–23)</label>
              <span className="text-xs text-muted-foreground font-mono">{context.current_hour ?? 0}:00</span>
            </div>
            <input
              type="range"
              min={0}
          ...
          {/* IP address */}
          <div className="space-y-1">
            <label className="text-xs font-medium text-foreground">IP Address (optional)</label>
            <input
              type="text"
              value={context.ip_address ?? ''}
              onChange={(e) => set('ip_address', e.target.value)}
              placeholder="192.168.1.1"
              className="w-full bg-muted border border-border rounded-lg px-3 py-1.5 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring font-mono"
            />
          </div>
              ...
              <div key={key} className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    context[key] ? 'bg-primary' : 'bg-muted'
                  )}
                >
                  <span
                    className={clsx(
          ...
          {/* User roles */}
          <div className="space-y-1">
            <label className="text-xs font-medium text-foreground">User Roles</label>
            <div className="flex flex-wrap gap-1.5 p-2 bg-muted border border-border rounded-lg min-h-[36px]">
              {(context.user_roles ?? []).map((role) => (
                ...
                >
                  {role}
                  <button type="button" onClick={() => removeRole(role)} className="text-primary hover:text-foreground">×</button>
                </span>
              ))}
              <input
                type="text"
                ...
                className="flex-1 min-w-[80px] bg-transparent text-xs text-foreground placeholder-muted-foreground focus:outline-none"
              />
            </div>
          </div>
          ...
          {/* Custom claims */}
          <div className="space-y-1">
            <label className="text-xs font-medium text-foreground">Custom Claims (optional)</label>
            <div className="space-y-1">
              {Object.entries(context.custom_claims ?? {}).map(([k, v]) => (
                <div key={k} className="flex items-center gap-2 text-xs">
                  <span className="font-mono text-foreground bg-muted px-2 py-0.5 rounded">{k}</span>
                  <span className="text-muted-foreground">=</span>
                  <span className="font-mono text-foreground bg-muted px-2 py-0.5 rounded flex-1 truncate">{v}</span>
                  <button type="button" onClick={() => removeClaim(k)} className="text-muted-foreground hover:text-destructive">×</button>
                </div>
              ))}
              <div className="flex gap-1.5">
                <input
                  ...
                  placeholder="key"
                  className="flex-1 bg-muted border border-border rounded px-2 py-1 text-xs text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring font-mono"
                />
                ...
                <button
                  type="button"
                  onClick={addClaim}
                  className="px-2 py-1 bg-muted hover:bg-accent text-foreground rounded text-xs transition-colors"
                >
                  Add
                </button>
              </div>
          ...
          <button
            ...
            onClick={handleRun}
            disabled={running}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-primary hover:bg-primary/90 disabled:opacity-50 text-primary-foreground rounded-xl text-sm font-medium transition-colors"
          >
            {running ? (
              <>
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          <h4 className="text-sm font-semibold text-foreground mb-4">Result</h4>
          ...
          {!result && !error && (
            <div className="flex flex-col items-center justify-center h-48 text-center">
              ...
              <p className="text-muted-foreground text-sm">Run a simulation to see results here</p>
            </div>
          )}
...
function SimulationResult({ result }: { result: SimulateResponse }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={clsx('p-4 rounded-xl border', dc.bg)}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{dc.desc}</p>
          </div>
        </div>
      </div>
      ...
      {/* Group trace */}
      <div className="space-y-2">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Group Trace</p>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className={group.matched ? 'text-primary' : 'text-muted-foreground'}>
                  {group.matched ? '✓' : '—'}
                </span>
                <span className="text-sm font-medium text-foreground">{group.display_name}</span>
              </div>
              <span
                className={clsx(
            ...
            <div className="pl-4 space-y-1">
              {group.rules.map((rule) => (
                <div key={rule.rule_id} className="flex items-center gap-2 text-xs">
                  <span className={rule.matched ? 'text-emerald-400' : 'text-muted-foreground'}>
                    {rule.matched ? '✓' : '✗'}
                  </span>
                  <span className={rule.matched ? 'text-foreground' : 'text-muted-foreground'}>
                    {rule.display_name}
                  </span>
                  <span className={clsx('ml-auto', rule.matched ? 'text-emerald-500' : 'text-muted-foreground')}>
